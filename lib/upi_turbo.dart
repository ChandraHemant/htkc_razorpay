...
class UpiTurbo {
  ...
  void linkNewUpiAccount({required String? customerMobile,  String? color , required OnSuccess<List<UpiAccount>> onSuccess,
    required OnFailure<RazorpayError> onFailure} ) async {
    try {
      ...
      if(getLinkedUpiAccountsResponse["data"]!=""){
        ...
        onFailure(RazorpayError(errorCode:"NO_ACCOUNT_FOUND" , errorDescription: "No Account Found"));
      }

    } on PlatformException catch (error) {
      onFailure(RazorpayError(errorCode:error.code , errorDescription: error.message!));
    }
  }

  void manageUpiAccounts({required String? customerMobile, String? color, required OnFailure<RazorpayError> onFailure} ) async {
    try {
      if(!_isTurboPluginAvailable){
        _emitFailure(onFailure);
        return;
  ...
  void _emitFailure(OnFailure<RazorpayError> onFailure) {
    onFailure(RazorpayError(errorCode:"AXIS_SDK_ERROR" , errorDescription: "No Turbo Plugin Found"));
  }
