...
Flutter plugin for HCRazorpay SDK.
...
- [API](#api)
- [Example App](https://github.com/ChandraHemant/htkc_razorpay/tree/main/example)
...
This plugin is available on Pub: [https://pub.dev/packages/htkc_razorpay](https://pub.dev/packages/htkc_razorpay)
...
The event names are exposed via the constants `EVENT_PAYMENT_SUCCESS`, `EVENT_PAYMENT_ERROR` and `EVENT_EXTERNAL_WALLET` from the `HCRazorpay` class.

Use the `on(String event, Function handler)` method on the `HCRazorpay` instance to attach event listeners.

```dart

_razorpay.on(HCRazorpay.EVENT_PAYMENT_SUCCESS, _handlePaymentSuccess);
_razorpay.on(HCRazorpay.EVENT_PAYMENT_ERROR, _handlePaymentError);
_razorpay.on(HCRazorpay.EVENT_EXTERNAL_WALLET, _handleExternalWallet);
```
...
To clear event listeners, use the `clear` method on the `HCRazorpay` instance.
...
### `CocoaPods could not find compatible versions for pod "htkc_razorpay"` when running `pod install`

```
Specs satisfying the `htkc_razorpay (from
    `.symlinks/plugins/htkc_razorpay/ios`)` dependency were found, but they
...
### iOS build fails with `'htkc_razorpay/htkc_razorpay-Swift.h' file not found`

Add use_frameworks! in `ios/Podfile` and run `pod install` again in the `ios` directory.

### Gradle build fails with `Error: uses-sdk:minSdkVersion 16 cannot be smaller than version 19 declared in library [:htkc_razorpay]`
...
### A lot of errors saying `xxxx is not defined for the class 'HCRazorpay'`

We export a class `HCRazorpay` from `package:htkc_razorpay/htkc_razorpay.dart`. Check if your code is redeclaring the `HCRazorpay` class.

### Type 'xxxx' is not a subtype of type 'xxxx' of 'response' in `HCRazorpay.on.<anonymous closure>`
...
#0      HCRazorpay.on.<anonymous closure> (package:htkc_razorpay/htkc_razorpay.dart:87:14)
...
#4      HCRazorpay._handleResult (package:htkc_razorpay/htkc_razorpay.dart:81:19)
#5      HCRazorpay.open (package:htkc_razorpay/htkc_razorpay.dart:49:5)
```
...
The error codes have been exposed as integers by the `HCRazorpay` class.
...
| INVALID_OPTIONS   | An issue with options passed in `HCRazorpay.open`                      |
...
The event names have also been exposed as Strings by the `HCRazorpay` class.
